...
<script>
  ...
  import {getMenuTree,menuEdit,getMenuById} from "@/services/sysMenu"
  export default {
    ...
    methods: {
      ...
      edit (id) {
        ...
        getMenuById({id: id}).then(res => {
          const data = res.data
          // 菜单层级赋值
          this.form.getFieldDecorator('type', {initialValue: data.type.toString() })
          this.meneTypeFunc(data.type.toString())
          setTimeout(() => {
      ...
      setMenuItem(data){
        this.form.setFieldsValue(
          ...
          this.form.getFieldDecorator('icon', { initialValue: data.icon })
        this.form.getFieldDecorator('parentId', { initialValue: data.parentId })
        if(data.type.toString() === '1'){
          this.permissionsData = data.permissionsData ? data.permissionsData : []
        }
      },
